...
#'
#' @param data Data with observed variable, optical bands, SAR backscatter (VV, VH, HH, HV), and soil nitrogen (Ns, if another variable is used, as Lidar, the name should be Ns)
#' @param y Observed variable
#' @param n_bands Number of optical bands to be combined for vegetation indices calculated from combinations of spectral bands, SAR backscatter, and nitrogen soil
...
#' )
#' 
optical_SAR_Ns_indices <- function(data, y, n_bands, set_bands, scan = FALSE) {
        ...
        for (j in (i+1):n) {
          ...
          if (grepl("1Ns", colnames(setbands)[i]) || grepl("1Ns", colnames(setbands)[j])) {
            if (grepl("1V", colnames(setbands)[i]) || grepl("1H", colnames(setbands)[i]) || grepl("1V", colnames(setbands)[j]) || grepl("1H", colnames(setbands)[j])) {
              ...
              cor_val <- suppressWarnings(abs(cor(y, VI, use = "complete.obs", method = 'spearman')))
